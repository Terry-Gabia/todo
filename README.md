...
Slack 봇과 양방향 연동되며, Supabase Realtime으로 실시간 동기화됩니다.
...
│  │   React 19 + Vite 7  │  │  NestJS + Slack Bolt │                 │
│  │                      │  │                      │                 │
│  │  • Todo CRUD         │  │  • Slack 봇 (Socket)  │                 │
│  │  • Google OAuth      │  │  • 명령어 처리        │                 │
│  │  • 테마 토글          │  │  • 알림 발송          │                 │
...
[웹에서 할일 추가]
  → Supabase DB INSERT
  → Realtime 이벤트 발생
  → Backend가 감지 → Slack DM 알림 발송

[Slack에서 할일 추가]
  → Backend가 Slack 메시지 수신 (Socket Mode)
...
| **알림** | Slack Bot (Bolt for JavaScript, Socket Mode) |
...
│   │   │   └── Slack/SlackSettings.tsx        # Slack 알림 설정
│   │   ├── hooks/
│   │   │   ├── useAuth.ts       # 인증 상태 관리
...
Slack 봇으로 할일 추가/조회/완료 및 실시간 알림을 받을 수 있습니다.

#### 4-1. Slack App 생성 및 설정

1. [Slack API](https://api.slack.com/apps)에 접속하여 **Create New App** 클릭
2. **From scratch** 선택 → 앱 이름 입력 (예: `Todo Bot`) → 워크스페이스 선택
3. 왼쪽 메뉴에서 **Socket Mode** 클릭:
   - **Enable Socket Mode** 토글 ON
   - App-Level Token 이름 입력 (예: `socket-token`) → **connections:write** scope 추가 → **Generate**
   - 생성된 토큰 복사 → `SLACK_APP_TOKEN` (xapp-으로 시작)
...
2. **Bot Token Scopes** 섹션에서 다음 권한 추가:
   ...
   - `im:write` — DM 채널 열기
3. 페이지 상단의 **Install to Workspace** 클릭하여 앱 설치
4. 설치 후 표시되는 **Bot User OAuth Token** 복사 → `SLACK_BOT_TOKEN` (xoxb-으로 시작)

#### 4-3. Event Subscriptions 설정
...
2. **Messages Tab** 체크하여 DM 탭 활성화

#### 사용자별 알림 연동 방법

```
1. Slack에서 설치한 봇 앱에 DM 전송
2. "내아이디" 또는 "myid" 입력 → 봇이 Slack Member ID를 알려줌
3. 웹 앱에서 헤더의 "Slack 알림 설정" 클릭
4. 복사한 Member ID 입력 후 저장
5. 이후 할일 추가/완료/삭제 시 Slack DM 알림 수신
```
...
| `내아이디` 또는 `myid` | 내 Slack Member ID 확인 |

> **참고**: Slack 봇은 슬래시 명령어(`/`)가 아닌 DM 일반 메시지로 동작합니다.
...
| `SLACK_APP_TOKEN` | Slack App-Level Token | Slack API > Basic Information > App-Level Tokens |
...
### 실시간 동기화
- Supabase Realtime으로 웹 ↔ Slack 양방향 동기화
...
CREATE TABLE user_profiles (
  ...
  slack_user_id    text           -- Slack Member ID (nullable)
);
```

- **RLS (Row Level Security)** 활성화: 유저는 자신의 데이터만 접근 가능
- **Realtime**: `todos` 테이블 변경 시 WebSocket으로 즉시 전파
- **Service Role**: 백엔드(Slack 봇)는 Service Role 키로 RLS 우회
...
| 백엔드 | Railway | 24시간 자동 실행 (Slack 봇 Socket Mode) |
